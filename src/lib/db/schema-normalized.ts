...
// User types table - User roles and account types combined
...
// Account types table - Account type lookup (for backward compatibility)
export const accountTypes = pgTable('account_types', {
  ...
  maxProducts: integer('maxProducts').default(0),
  createdAt: timestamp('createdAt', { withTimezone: true }).defaultNow(),
});
...
// Users table - Core user accounts with foreign keys to lookup tables
export const users = pgTable('users', {
  id: uuid('id').primaryKey().defaultRandom(),
  email: text('email').notNull().unique(),
  name: text('name').notNull(),
  passwordHash: text('passwordHash').notNull(),
  userTypeId: uuid('userTypeId').notNull().references(() => userTypes.id),
  accountTypeId: uuid('accountTypeId').notNull().references(() => accountTypes.id),
...
// User business table - Junction table for user-business relationships
export const userBusiness = pgTable('user_business', {
  id: uuid('id').primaryKey().defaultRandom(),
  userId: uuid('userId').notNull().references(() => users.id, { onDelete: 'cascade' }),
  businessId: uuid('businessId').notNull().references(() => businessDetails.id, { onDelete: 'cascade' }),
...
export const products = pgTable('products', {
  ...
  name: text('name').notNull(),
  description: text('description'),
  packageSize: text('packageSize'), // Combined quantity and unit (e.g., "50 kg", "25 bags")
...
export const usersRelations = relations(users, ({ one, many }) => ({
  ...
  accountType: one(accountTypes, { fields: [users.accountTypeId], references: [accountTypes.id] }),
...
export const userBusinessRelations = relations(userBusiness, ({ one }) => ({
  user: one(users, { fields: [userBusiness.userId], references: [users.id] }),
  business: one(businessDetails, { fields: [userBusiness.businessId], references: [businessDetails.id] }),
}));
...
export type UserBusiness = typeof userBusiness.$inferSelect;
export type NewUserBusiness = typeof userBusiness.$inferInsert;
